...
**Implementation Note**: The docker-compose configuration in this repository has been adapted from this guide with the following changes:
- Uses `sickcodes/docker-osx:latest` (for setup) and `:naked` (for production) images from Docker Hub
- Specifies macOS version via `SHORTNAME=monterey` environment variable instead of version-specific image tags
- Version-specific tags like `:ventura` shown in this guide are for local builds only and don't exist on Docker Hub
- The setup steps remain functionally the same
